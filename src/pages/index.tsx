...
export default function Home() {
  ...
  async function getAllPokemons() {
    try {
      ...
      const list = await api.get(
        `/pokemon/?limit=${ROWS_PER_PAGE}&offset=${page * ROWS_PER_PAGE}`
      );
  ...
  async function searchPokemon() {
    try {
      setLoading(true);
      const list = await api.get(
        `/pokemon/${pokemonPesquisa?.trim().toLowerCase()}`
      );
  ...
  return (
    <DefaultLayout>
      ...
      <Corpo>
        ...
        >
          <AlertInfo infoAlert={infoAlert} setInfoAlert={setInfoAlert} />
          <ContainerFlex width="70%" fd="column">
            ...
            >
              ...
              {loading ? (
                <Skeleton w="183px" h="60px" />
              ) : (
                <Button onClick={() => setInfoAlert('Função não implementada')}>
                  Novo Pokémon
                </Button>
              )}
            </ContainerFlex>
            ...
            >
              {loading &&
                [...Array(ROWS_PER_PAGE).keys()].map((item, index) => {
                  return <Skeleton key={`sk-${index}`} />;
                })}
